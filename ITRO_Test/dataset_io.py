...
def resolve_dataset_path(name_or_path):
    ...
    checking the path as given first, then datasets/ within ITRO_Test.
...
def write_results(dataset_path, results, summary):
    ...
    <dataset_stem>_itro_answers.json, right next to the input dataset
    ...
    out_path = os.path.join(out_dir, f"{stem}_itro_answers.json")
