...
def _llm_score(query_text, domain, provider, max_retries=3):
    """
    Ask the LLM to score the 4 tau dimensions.
    Retries up to max_retries on JSON parse failure —
    local models occasionally produce malformed JSON.
    Returns a dict of scores or None on total failure.
    """
    base_prompt = _build_scorer_prompt(query_text, domain)
    ...
    for attempt in range(max_retries):
        ...
        if attempt == 0:
            ...
            # Explicit reminder on retry
            retry_note = (
                f"\n\nIMPORTANT: Your previous response was not valid JSON. "
                f"Reply with ONLY the JSON object. "
                f"No markdown fences, no explanation, "
                f"no text before or after the braces."
            )
            full_prompt = f"{SCORER_SYSTEM}{retry_note}\n\n---\n\n{base_prompt}"

        try:
            raw    = provider.call(full_prompt, max_tokens=150)
